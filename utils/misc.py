...
import torchvision
# if float(torchvision.__version__[:-8]) < 0.7:
...
def collate_fn3d(raw_batch):
    raw_batch = list(zip(*raw_batch))
    
    points = torch.stack(raw_batch[0])
    points_data = NestedTensor(points, None)
    word_id = torch.tensor(raw_batch[1])
    word_mask = torch.tensor(raw_batch[2])
    text_data = NestedTensor(word_id, word_mask)
    bbox = torch.tensor(raw_batch[3])
    parameters = raw_batch[4]
    batch = [points_data, text_data, bbox, parameters]
    return tuple(batch)
