...
from utils.box_utils import bbox_iou, xywh2xyxy, xyxy2xywh, generalized_box_iou, generalized_box_iou3d
...
def trans_vg_loss(batch_pred, batch_target):
    ...
    if batch_pred.shape[1] == 4:
        loss_giou = 1 - torch.diag(generalized_box_iou(
            xywh2xyxy(batch_pred),
            xywh2xyxy(batch_target)
        ))
    else:
        loss_giou = 1 - generalized_box_iou3d(
            batch_pred[:, :6],
            batch_target[:, :6]
        )
    losses = {}
