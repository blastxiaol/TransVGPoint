...
class RandomResize(object):
    ...
    def __call__(self, input_dict):
        ...
        if self.with_long_side:
            resized_img, resized_box, ratio = resize_according_to_long_side(img, box, size)
        else:
            resized_img, resized_box, ratio = resize_according_to_short_side(img, box, size)
...
class NormalizeAndPad(object):
    def __init__(self, mean=[0.492, 0.459, 0.433], std=[0.257, 0.262, 0.265], size=640, aug_translate=False):
        self.mean = mean
        self.std = std
        self.size = size
