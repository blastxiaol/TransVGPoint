...
class SUNREFER2D(data.Dataset):
    def __init__(self, transform=None, return_idx=False, testmode=False, 
    ...
    def pull_item(self, idx):
        object_id = int(self.sunrefer[idx]['object_id'])
        bbox = np.array(self.sunrefer[idx]['boxes2d'][object_id], dtype=np.float32)

        phrase = self.sunrefer[idx]['sentence']

        img_path = self.sunrefer[idx]['image_path']
